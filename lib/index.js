...
/** Render a SKILL.md document from edited fields. */
function renderSkillMarkdown(input) {
  const lines = ['---', `name: ${stringifyYaml(input.name)}`, `description: ${stringifyYaml(input.description)}`]
  if (input.whenToUse !== undefined && input.whenToUse !== '') lines.push(`whenToUse: ${stringifyYaml(input.whenToUse)}`)
...
async function installUserSkill(input) {
  const name = input.name
  if (!isSkillName(name)) throw new ManageError('invalid-skill-name', `invalid skill name "${name}" (kebab-case required)`)
  const root = skillsRoot()
  ...
  await writeFile(target, renderSkillMarkdown(input), 'utf8')
  return name
}
...
function routes(ctx) {
  return {
    ...
    [`${ROUTE_BASE}/skill.install`]: async (payload) => ({ value: { name: await installUserSkill(payload) } }),
