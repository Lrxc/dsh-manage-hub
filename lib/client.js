// DeepSeek Harness plugin "mcp-skills" — browser half.
//
...
// the plugin's own /dsh-mcp-skills routes (no dependency on the core api
// client, so no core patching is needed to install this plugin).
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── styles ────────────────────────────────────────────────────────────────
    var manageStylesId = 'dsh-mcp-skills/manage.module.css'
    var manageCss =
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css=' + JSON.stringify(manageStylesId) + ']') === null) {
      ...
      tag.dataset.plugin = 'dsh-mcp-skills'
    ...
    /**
     * POST one /dsh-mcp-skills route and shape the reply like the core api
     * client ({ result: { ok, value | error } }) so the section components stay
     * transport-agnostic.
     */
    async function callHost(path, payload) {
      const response = await fetch('/dsh-mcp-skills/' + path, {
        method: 'POST',
    ...
    /** Default per-call timeout, matching @deepseek-ai/dsh-mcp-client. */
    var MANAGE_DEFAULT_TIMEOUT_MS = 60000
    ...
    /**
     * Parse textarea lines into a record. Separators are matched by earliest
     * position, so a value may contain the other one; with `bareAsKey` a line
     * without any separator becomes a key with an empty value (env form).
     */
    function managePairs(text, separators, bareAsKey) {
      var result = {}
      var lines = manageLines(text)
      for (var i = 0; i < lines.length; i++) {
        var line = lines[i]
        var index = -1
        for (var s = 0; s < separators.length; s++) {
          var at = line.indexOf(separators[s])
          if (at !== -1 && (index === -1 || at < index)) index = at
        }
        if (index === -1) {
          if (bareAsKey) result[line] = ''
          continue
        }
        if (index === 0) continue
    ...
    function manageKeyValue(text) {
      return managePairs(text, ['='], true)
    }

    /** HTTP headers: `Name: Value` per line (`Name=Value` kept for compatibility). */
    function manageHeaders(text) {
      return managePairs(text, [':', '='], false)
    }

    /** Render one stored header back into the textarea's `Name: Value` form. */
    function manageHeaderLine(key, value) {
      if (value === '') {
        // Written by the earlier version that split on `=`: the whole
        // `Name: Value` line was stored as the key. Recover the pair.
        var index = key.indexOf(':')
        if (index > 0) return key.slice(0, index).trim() + ': ' + key.slice(index + 1).trim()
      }
      return key + ': ' + value
    }
    ...
    function mcpDraftToServer(draft) {
      return {
        ...
        toolCallTimeoutMs: Number(draft.toolCallTimeoutMs) || MANAGE_DEFAULT_TIMEOUT_MS,
        ...
        ...(draft.transport === 'stdio'
          ...
          : { url: draft.url, headers: manageHeaders(draft.headersText) }),
      }
    }
    ...
    function mcpServerToDraft(server) {
      return {
        ...
        cwd: server.cwd ?? '',
        url: server.url ?? '',
        headersText: Object.entries(server.headers ?? {}).map(([key, value]) => manageHeaderLine(key, value)).join('\n'),
        toolCallTimeoutMs: String(server.toolCallTimeoutMs ?? MANAGE_DEFAULT_TIMEOUT_MS),
    ...
    function SkillsSection({ api, t }) {
      ...
      var handleImportFile = useManageCallback(async (file) => {
        ...
        try {
          ...
          var skippedNames = response.result.value.skipped ?? []
          if (importedNames.length === 0) {
            setNotice(t('skills.importedNone').replace('{count}', String(skippedNames.length)))
          } else if (skippedNames.length > 0) {
            setNotice(
              t('skills.importedSome').replace('{count}', String(importedNames.length)).replace('{skipped}', String(skippedNames.length)),
            )
          } else if (importedNames.length > 1) {
            setNotice(t('skills.importedMany').replace('{count}', String(importedNames.length)))
          } else {
            setNotice(t('skills.imported'))
          }
          await load()
        } catch (cause) {
          setError(manageMessageOf(cause))
    ...
    // ── copy dictionaries ──────────────────────────────────────────────────────
    var MANAGE_NS = 'settings-mcp-skills'
    var manageZh = {
      'skills.nav': 'Skills',
      ...
      'skills.importedSome': '已导入 {count} 个技能，跳过 {skipped} 个（已存在或不支持）。',
      'skills.importedNone': '未导入任何技能：{count} 个已存在或不支持。',
      'skills.removed': '已删除。',
      'skills.enabled': '允许模型调用',
      'mcp.nav': 'MCP',
      'mcp.title': 'MCP 服务器',
      'mcp.refresh': '刷新',
      'mcp.new': '添加服务器',
    ...
    var manageEn = {
      ...
      'skills.importedSome': 'Imported {count} skill(s); skipped {skipped} (already installed or unsupported).',
      'skills.importedNone': 'Nothing imported: {count} skill(s) already exist or are unsupported.',
      'skills.removed': 'Removed.',
      'skills.enabled': 'Model invocation',
      'mcp.nav': 'MCP',
