...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * The Skills settings page: user-owned skill catalog with import / edit /
    ...
    function SkillsSection({ api, t }) {
      ...
      var [importing, setImporting] = useManageState(false)
      var [importFile, setImportFile] = useManageState(null)
      ...
      var openImport = useManageCallback(() => {
        setImporting(true)
        setImportFile(null)
        setEditing(null)
        setError(null)
      ...
      var submit = useManageCallback(async () => {
        ...
        try {
          ...
          var response = await manageApi.update(payload)
          if (!response.result.ok) throw new Error(response.result.error.message)
          setEditing(null)
          setDraft(null)
      ...
      var submitImport = useManageCallback(async () => {
        if (importFile === null) return
        setBusy(true)
        setError(null)
        setNotice(null)
        try {
          var data = await readFileAsBase64(importFile)
          var response = await manageApi.install({ data, filename: importFile.name })
          if (!response.result.ok) throw new Error(response.result.error.message)
          setImporting(false)
          setImportFile(null)
      ...
      return react_jsx_runtime.jsxs('div', {
        ...
        children: [
          ...
          importing || editing !== null
            ? react_jsx_runtime.jsxs('div', {
                className: manageStyles.form,
                children: [
                  react_jsx_runtime.jsx('div', {
                    className: manageStyles.title,
                    children: importing ? t('skills.install.title') : t('skills.edit.title').replace('{name}', editing.name),
                  }),
                  importing
                    ? [
                        ...
                        react_jsx_runtime.jsx('div', {
                          className: manageStyles.hint,
                          children: t('skills.import.hint'),
                        }),
                      ]
                    : [
                    ...
                    children: [
                      importing
                        ? react_jsx_runtime.jsx(_deepseek_ai_dsh_client_ui_primitives.Button, {
                            ...
                            children: busy ? t('skills.import.busy') : t('skills.import.submit'),
                          })
                        : react_jsx_runtime.jsx(_deepseek_ai_dsh_client_ui_primitives.Button, {
                            variant: 'primary',
                            size: 'sm',
    ...
    var manageZh = {
      ...
      'skills.import.hint': '上传一个 zip 压缩包（内含 SKILL.md）。',
      'skills.import.submit': '导入',
      'skills.import.busy': '导入中…',
    ...
    var manageEn = {
      ...
      'skills.import.hint': 'Upload a zip archive containing a SKILL.md.',
      'skills.import.submit': 'Import',
      'skills.import.busy': 'Importing…',
