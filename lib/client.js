...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function callHost(path, payload) {
      return fetch('/dsh-mcp-skills/' + path, {
        method: 'POST',
    ...
    /** Parse `KEY=VALUE` lines into a record (environment variables). */
    ...
    /**
     * Parse `Name: Value` lines into a record (HTTP headers). `Name=Value` is
     * accepted too, since an earlier version wrote headers through the env
     * parser; whichever separator comes first decides the split.
     */
    function manageHeaders(text) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        var index = colon === -1 ? equals : equals === -1 ? colon : Math.min(colon, equals)
        if (index <= 0) continue
